...
10. marks tags containing `-` as prereleases; other version tags are stable releases.

Create a release after the intended commit is on `main`. Replace `X.Y.Z` with the version in `package.json`:
